...
def buy():
    if request.method == 'POST':
        ...
        try:
            ...
            flash(f"Error: could not find stock name for {stock_symbol}")
        ...
        try:
            ...
            flash(f"{stock_symbol}: No data found, symbol may be delisted")
            return redirect(url_for("transactions.main"))
    
        try:
            price = get_price(fast_info,stock_symbol, date)
        except:
            flash('Please Type a Date.')
            return redirect(url_for("transactions.main"))
        ...
        try:
            ...
            flash("Please enter a quantity greater than zero.")
            return redirect(url_for("transactions.main"))
        ...
        if not check_balance(amount):
            flash("Insufficient funds in account to complete purchase.")
            return redirect(url_for("transactions.main"))
...
def get_price(fast_info, stock_symbol, date):
    if date > datetime.now().date():
        flash("You can't select a date from the future! Please select a different date.")
        raise Exception
        ...
        if price == None:
            flash(f"Data was not found for {stock_symbol} Or Not a Trading Day")
            raise Exception
        else:
            return price[0]
...
def USD2Share(amountUSD,price):
    if amountUSD=='':
        flash("Please type a amount of USD or shares.")
        redirect(url_for("transactions.main"))
    try:
        quantity=float(amountUSD)//price
    except:
        flash('Please type a number of amount.')
        redirect(url_for("transactions.main"))
    return int(quantity)
...
def sell():
    if request.method == 'POST':
        ...
        if quantity <= 0 or quantity > quantity_owned:
            flash(f"Please enter a quantity greater than zero and less than or equal to the number of shares you own for {stock_symbol}")
            return redirect(url_for("transactions.main"))
...
def calculate_efficient_frontier(symbols,shares_owned):
    ...
    currentWeight=pd.DataFrame(shares_owned,index=expected_rts.index).T*adj_close.iloc[-1,:]
